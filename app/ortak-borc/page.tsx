...
import { Wallet, Calendar, Users, FileText } from "lucide-react";

interface OrtakBorc {
  _id: string;
  borclu_id: { _id: string; name: string };
  alacakli_id: { _id: string; name: string };
  borc_turu: { _id: string; ad: string };
  miktar: number;
  birim: { _id: string; ad: string };
  borc_tutari: number;
  odeme_durumu: string;
  notlar: string;
  odeme_sekli?: string;
  odeme_tarihi?: string;
  onaylandi: boolean;
  paylar: Array<{ sahip_id: { _id: string; name: string }; pay: number }>;
  taksit_sayisi: number;
  ilk_taksit_tarihi: string;
...
export default function OrtakBorcPage() {
  ...
  const fetchBorcTurleri = async () => {
    try {
      ...
      console.error("Borç türleri getirme hatası:", error);
      toast({
        title: "Hata",
  ...
  const fetchBirimler = async () => {
    try {
      ...
      console.error("Birimler getirme hatası:", error);
      toast({
        title: "Hata",
      ...
      <div className="container mx-auto p-4 min-h-screen bg-gray-900/50">
        <div className="mb-8">
          ...
          <p className="text-center text-neon-blue opacity-80">
            Ortak borçları yönetin ve takip edin
          </p>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="benim-borclarim" className="mt-0">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {ortakBorclar
                .filter((borc) => borc.borclu_id._id === session?.user?.id)
                .map((borc) => (
                  <Card
                    key={borc._id}
                    className="bg-gray-800/50 border-2 border-neon-purple hover:border-neon-pink transition-all duration-300"
